...
module Sass::Script::Tree
  ...
  class Funcall < Node
    ...
    def _to_sexp(visitor)
      return ruby_function_call(visitor) if Sass::Script::Functions.callable?(ruby_name)

      variable, function = visitor.environment.fn_variable(name)
      return visitor.run_callable(variable, function, self, "function #{name}")
    end

    def ruby_function_call(visitor)
      unless (signature = Sass::Script::Functions.signature(ruby_name.to_sym, args.size, keywords.size))
        unless keywords.empty? && kwarg_splat.nil?
      ...
      if signature.var_kwargs && !signature.var_args && args.size > signature.args.size
        ...
        sass_error(s(:dstr, '', s(:evstr, args[signature.args.size].to_sexp),
      ...
      arg_sexps = args.map do |arg|
        ...
        if keywords.has_key?(argname)
          ...
          sass_error("Function #{name} requires an argument named $#{argname}.")
        end
      end
    ...
    def simple_ruby_function_call(visitor)
      ...
      call << s(:splat, s(:call, splat.to_sexp(visitor)) if splat
      return call
    end
